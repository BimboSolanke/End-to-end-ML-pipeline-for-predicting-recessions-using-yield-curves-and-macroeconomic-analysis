# -*- coding: utf-8 -*-
"""_Part3_Updated_recession_prediction_analysis_python_notebook_finalVer_BimboSolanke.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/19Sg_LQwJca42b7d9iH6iHNCQ8txLd2iR
"""
...
from sklearn.metrics import (roc_auc_score, roc_curve, confusion_matrix,
                           ...
                           precision_recall_curve, log_loss, make_scorer, balanced_accuracy_score, average_precision_score)
...
fig1.suptitle('Comprehensive Data Overview - Recession Prediction Dataset',
...
g.fig.suptitle('Pairwise Relationships of Key Economic Indicators',
...
fig4.suptitle('Yield Curve Dynamics and Recession Indicators',
              fontsize=20, fontweight='bold')
...
# Test multiple yield spreads (original three)
...
# Additional yield curve inversion tests
print("\n" + "-"*60)
print("ADDITIONAL YIELD CURVE INVERSION TESTS")
print("-"*60)

additional_spreads = ['TS_5y2y', 'TS_2y3m', 'TS_30y10y', 'TS_30y5y', 'TS_30y2y']
for spread in additional_spreads:
    if spread in df_comprehensive.columns:
        # Create inversion indicator for this spread
        df_comprehensive[f'{spread}_inverted'] = (df_comprehensive[spread] < 0).astype(int)
        results = test_hypothesis(df_comprehensive, f'{spread}_inverted')
...
# visibility of imbalance handling
print("\nImbalance handling:")
print(" - Logistic-family models: class_weight='balanced'")
print(" - RandomForest: class_weight='balanced'")
print(" - XGBoost: scale_pos_weight = N_neg / N_pos")


# =============================================================================
# PART C: MULTICOLLINEARITY CHECK (VIF)
# =============================================================================
# defining a single, shared time-aware CV splitter for all models
...
print("   Class-imbalance setting: class_weight='balanced'")
...
# 5. RANDOM FOREST (tuned with randomized search and time-series CV)
print("\n5. Random Forest (tuned)")

# param distributions for a broader search
param_distributions = {
    ...
    'class_weight': ['balanced']  # keep imbalance handling explicit
}

rf_base = RandomForestClassifier(random_state=42, oob_score=False)  # OOB off during CV

# using the same time-aware CV already defined
rf_search = RandomizedSearchCV(
    estimator=rf_base,
    param_distributions=param_distributions,
    n_iter=60,
    scoring='roc_auc',
    cv=tscv,               #the TimeSeriesSplit from PART E
    verbose=0,
    random_state=42,
    n_jobs=-1
)

rf_search.fit(X_train_scaled, y_train_min)
rf_best = rf_search.best_estimator_

# fit best model on train
rf_best.fit(X_train_scaled, y_train_min)
print("   Class-imbalance setting: class_weight='balanced'")

y_pred_rf = rf_best.predict_proba(X_test_scaled)[:, 1]
auc_rf = roc_auc_score(y_test_min, y_pred_rf)
print(f"   Best params: {rf_search.best_params_}")
print(f"   AUC: {auc_rf:.3f}")

model_results['RandomForest'] = {'model': rf_best, 'auc': auc_rf, 'predictions': y_pred_rf}
...
print(f"   Class-imbalance setting: scale_pos_weight={xgb.get_xgb_params()['scale_pos_weight']:.2f}")
...
for name, data in model_results.items():
    scorers = {
    'AUC': 'roc_auc',
    'AP': make_scorer(average_precision_score, needs_threshold=True)
}

for metric_name, scorer in scorers.items():
    scores = cross_val_score(data['model'], X_train_scaled, y_train_min, cv=tscv, scoring=scorer)
    print(f"{name:15s}: CV {metric_name} = {scores.mean():.3f} ± {scores.std():.3f}")
...
for name, results in model_results.items():
    ...
    ap = average_precision_score(y_test_min, results['predictions'])
...
for name, results in model_results.items():
    ...
    # Average Precision (PR-AUC) – consistent metric for imbalance
    comparison_df.loc[name, 'AP'] = average_precision_score(y_test_min, results['predictions'])

    # Balanced Accuracy
    comparison_df.loc[name, 'BalancedAcc'] = balanced_accuracy_score(y_test_min, y_pred_binary)

    # Specificity (TNR)
    specificity = tn / (tn + fp) if (tn + fp) > 0 else 0
    comparison_df.loc[name, 'Specificity'] = specificity
...
# =============================================================================
# PART B.5: EXPANDING-WINDOW HISTORICAL PROBABILITIES (Parsimonious Logit)
# =============================================================================
# Uses ONLY a 3-feature logistic model to generate one-step-ahead probabilities
# across history, training strictly on data available up to each time t.

print("\n" + "="*80)
print("PART B.5: EXPANDING-WINDOW HISTORICAL PROBABILITIES (Parsimonious Logit)")
print("="*80)

features_min_ew = ['TS_10y3m', 'CreditSpread', 'Unemp_D12']   # 3-predictor logit
target_ew = 'Recession_12m'

# Align to common monthly index (safety)
# 1) Base index from engineered features
idx_features = df_minimal.index

# 2) Get a reliable USREC source: prefer in-memory DataFrame named `df`; else reload raw CSV
_use_df = (
    'df' in globals()
    and isinstance(df, pd.DataFrame)
    and 'USREC' in df.columns
...
if _use_df:
    ...
    # fallback: read the raw data that Stage 1 already saved
    usrec_src = pd.read_csv('recession_raw_data.csv', index_col='DATE', parse_dates=True)

# 3) Build common index and aligned slices
common_idx = idx_features.intersection(usrec_src.index).sort_values()
X_full_ew  = df_minimal.loc[common_idx, features_min_ew].copy()
y_full_ew  = df_minimal.loc[common_idx, target_ew].copy()
usrec_ew   = usrec_src.loc[common_idx, 'USREC'].astype(int)

#2) Build expanding-window probabilities (no look-ahead)
from sklearn.preprocessing import StandardScaler
from sklearn.linear_model import LogisticRegression

min_train_months = 120  # ~10 years to stabilize coefficients
proba_series = pd.Series(index=common_idx, dtype=float)

for t in range(min_train_months, len(common_idx)):
    # Train strictly on data BEFORE time t
    train_idx = common_idx[:t]
    test_idx  = common_idx[t:t+1]

    X_train, y_train = X_full_ew.loc[train_idx], y_full_ew.loc[train_idx]
    X_test           = X_full_ew.loc[test_idx]

    scaler_ew = StandardScaler()
    X_train_s = scaler_ew.fit_transform(X_train)
    X_test_s  = scaler_ew.transform(X_test)

    logit_ew = LogisticRegression(
        penalty='l2', C=1.0, max_iter=1000,
        class_weight='balanced', random_state=42
    )
    logit_ew.fit(X_train_s, y_train)
    proba_series.iloc[t] = logit_ew.predict_proba(X_test_s)[:, 1][0]

# Optional display band (visual aid only; does not affect results)
ci_lo = proba_series.rolling(6, min_periods=1).quantile(0.10)
ci_hi = proba_series.rolling(6, min_periods=1).quantile(0.90)

#3) Plot
fig, ax = plt.subplots(figsize=(14, 5))

# Probability line
ax.plot(proba_series.index, proba_series.values, linewidth=2,
        label='Predicted Probability (Logit, 12-month ahead)', color='black')

# Confidence band (visual aid)
ax.fill_between(proba_series.index, ci_lo, ci_hi, alpha=0.15, color='red',
                label='Rolling 10–90% band (display)')

# Shade NBER recessions
in_rec = False
start = None
for dt, val in usrec_ew.items():
    if val == 1 and not in_rec:
        start = dt
        in_rec = True
    elif val == 0 and in_rec:
        ax.axvspan(start, dt, color='gray', alpha=0.25)
        in_rec = False
# If series ends in recession
if in_rec:
    ax.axvspan(start, usrec_ew.index[-1], color='gray', alpha=0.25)

ax.set_ylim(0, 1.0)
ax.set_ylabel('Probability of Recession (next 12 months)')
ax.set_title('Expanding-Window Predicted Recession Probability vs. NBER Recessions')
ax.legend(loc='upper left', frameon=True)
ax.grid(alpha=0.3)

plt.tight_layout()
plt.savefig('figure_expanding_logit_prob_vs_recessions.png', dpi=300, bbox_inches='tight')
plt.show()

print("✓ Saved: 'figure_expanding_logit_prob_vs_recessions.png'")
...
3. Model assumes stable economic relationships
...
# === Standalone block to test all yield spreads for predictive power ===
df = pd.read_csv('recession_raw_data.csv', index_col='DATE', parse_dates=True)
...
if 'spreads' not in globals():
    ...
    spreads['30Y-10Y'] = df['DGS30'] - df['DGS10']
    spreads['30Y-5Y']  = df['DGS30'] - df['DGS5']
    spreads['30Y-2Y']  = df['DGS30'] - df['DGS2']

# 2. Define the lead range (0–24 months)
lead_months = range(0, 25)

# 3. Compute most negative (strongest) correlation and its lead for each spread
predictive_power = {}
best_leads = {}
for spread_name in spreads.columns:
    correlations = []
    ...
    # We look for the most negative correlation (strong strongest predictive signal)
    idx_min = correlations.argmin()
    predictive_power[spread_name] = correlations[idx_min]
    best_leads[spread_name] = lead_months[idx_min]

# 4. Rank spreads by predictive power (most negative first) and print results
sorted_spreads = sorted(predictive_power.items(), key=lambda x: x[1])  # ascending => most negative first
print("Predictive power of yield spreads (more negative correlation means stronger recession predictor):")
for spread_name, corr in sorted_spreads:
    print(f"{spread_name}: correlation={corr:.3f}, at {best_leads[spread_name]} months lead")

# 5. Plot a bar chart summarising the predictive power
fig, ax = plt.subplots(figsize=(10, 6))
spread_names_sorted = [x[0] for x in sorted_spreads]
corr_values_sorted = [x[1] for x in sorted_spreads]
bars = ax.barh(spread_names_sorted, corr_values_sorted, color=plt.cm.tab10(range(len(sorted_spreads))))

ax.set_xlabel('Most Negative Correlation with Future Recession')
ax.set_ylabel('Yield Spread')
ax.set_title('Predictive Power of Yield Spreads (Lead–Lag Correlation)')
ax.grid(axis='x', alpha=0.3)

# Annotate each bar with the lead time that gave the most negative correlation
for bar, spread_name in zip(bars, spread_names_sorted):
    lead = best_leads[spread_name]
    ax.text(bar.get_width(), bar.get_y() + bar.get_height() / 2, f"{lead}m",
            va='center', ha='left', fontsize=8)

plt.tight_layout()
plt.show()

# ENHANCED MODEL EVALUATION WITH OUTPUTS
# Set style for outputs
...
print("="*80)
print("COMPREHENSIVE MODEL EVALUATION WITH PRESENTATION FORMATTING")
print("="*80)

# [Previous model training code remains the same until results collection]
# --- Adding Model Training and Evaluation Code Here to Populate 'results' ---
...
y_pred_lr = lr.predict_proba(X_test_scaled)[:, 1]
auc_lr = roc_auc_score(y_test_min, y_pred_lr)
brier_lr = brier_score_loss(y_test_min, y_pred_lr)
results['Logistic'] = {'model': lr, 'auc': auc_lr, 'brier': brier_lr, 'predictions': y_pred_lr}
...
y_pred_ridge = ridge.predict_proba(X_test_scaled)[:, 1]
auc_ridge = roc_auc_score(y_test_min, y_pred_ridge)
brier_ridge = brier_score_loss(y_test_min, y_pred_ridge)
results['Ridge'] = {'model': ridge, 'auc': auc_ridge, 'brier': brier_ridge, 'predictions': y_pred_ridge}
...
y_pred_lasso = lasso.predict_proba(X_test_scaled)[:, 1]
auc_lasso = roc_auc_score(y_test_min, y_pred_lasso)
brier_lasso = brier_score_loss(y_test_min, y_pred_lasso)
results['Lasso'] = {'model': lasso, 'auc': auc_lasso, 'brier': brier_lasso, 'predictions': y_pred_lasso}
...
y_pred_elastic = elastic.predict_proba(X_test_scaled)[:, 1]
auc_elastic = roc_auc_score(y_test_min, y_pred_elastic)
brier_elastic = brier_score_loss(y_test_min, y_pred_elastic)
results['ElasticNet'] = {'model': elastic, 'auc': auc_elastic, 'brier': brier_elastic, 'predictions': y_pred_elastic}
...
y_pred_rf = rf_best.predict_proba(X_test_scaled)[:, 1]
auc_rf = roc_auc_score(y_test_min, y_pred_rf)
brier_rf = brier_score_loss(y_test_min, y_pred_rf)
results['RandomForest'] = {'model': rf_best, 'auc': auc_rf, 'brier': brier_rf, 'predictions': y_pred_rf}
...
xgb_clf = XGBClassifier(n_estimators=100, max_depth=3, learning_rate=0.1,
                   scale_pos_weight=len(y_train_min[y_train_min==0])/len(y_train_min[y_train_min==1]),
                   random_state=42, use_label_encoder=False, eval_metric='logloss')
xgb_clf.fit(X_train_scaled, y_train_min)

y_pred_xgb = xgb_clf.predict_proba(X_test_scaled)[:, 1]
auc_xgb = roc_auc_score(y_test_min, y_pred_xgb)
brier_xgb = brier_score_loss(y_test_min, y_pred_xgb)
results['XGBoost'] = {'model': xgb_clf, 'auc': auc_xgb, 'brier': brier_xgb, 'predictions': y_pred_xgb}


# --- Add Polynomial and Interaction Features Models ---
...
y_pred_lr_poly = lr_poly.predict_proba(X_test_poly_scaled)[:, 1]
auc_lr_poly = roc_auc_score(y_test_min, y_pred_lr_poly)
brier_lr_poly = brier_score_loss(y_test_min, y_pred_lr_poly)
results['Logistic_Poly'] = {'model': lr_poly, 'auc': auc_lr_poly, 'brier': brier_lr_poly, 'predictions': y_pred_lr_poly}
...
y_pred_lasso_poly = lasso_poly.predict_proba(X_test_poly_scaled)[:, 1]
auc_lasso_poly = roc_auc_score(y_test_min, y_pred_lasso_poly)
brier_lasso_poly = brier_score_loss(y_test_min, y_pred_lasso_poly)
results['Lasso_Poly'] = {'model': lasso_poly, 'auc': auc_lasso_poly, 'brier': brier_lasso_poly, 'predictions': y_pred_lasso_poly}
...
y_pred_lr_int = lr_int.predict_proba(X_test_int_scaled)[:, 1]
auc_lr_int = roc_auc_score(y_test_min, y_pred_lr_int)
brier_lr_int = brier_score_loss(y_test_min, y_pred_lr_int)
results['Logistic_Interactions'] = {'model': lr_int, 'auc': auc_lr_int, 'brier': brier_lr_int, 'predictions': y_pred_lr_int}
...
for name, model_data in results.items():
    ...
    # Find optimal threshold for metrics that require binary predictions
    ...
    model_data['Accuracy'] = accuracy_score(y_true, y_pred_binary)
    model_data['Precision'] = precision_score(y_true, y_pred_binary, zero_division=0)
    model_data['Recall'] = recall_score(y_true, y_pred_binary, zero_division=0)
    model_data['F1'] = f1_score(y_true, y_pred_binary, zero_division=0)
    model_data['BalancedAcc'] = balanced_accuracy_score(y_true, y_pred_binary)
    model_data['Specificity'] = tn / (tn + fp) if (tn + fp) > 0 else 0

# Convert results dictionary to DataFrame
df_results = pd.DataFrame(results).T[['auc', 'Accuracy', 'Precision', 'Recall', 'F1', 'brier', 'BalancedAcc', 'Specificity']].rename(columns={'auc': 'AUC', 'brier': 'Brier'})


# After collecting all results, create visualizations

# 1. CREATE STYLED DATAFRAME
def style_dataframe(df):
    """Apply presentation-ready styling to dataframe"""

    # Color mapping for performance
    def color_auc(val):
        if val >= 0.79:
            return 'background-color: #2ecc71; font-weight: bold; color: white'
        elif val >= 0.77:
            return 'background-color: #3498db; font-weight: bold; color: white'
        else:
            return 'background-color: #95a5a6; color: white'

    def color_accuracy(val):
        if val >= 0.78:
            return 'background-color: #27ae60; font-weight: bold; color: white'
        elif val >= 0.76:
    ...
    def color_brier(val):
        if val <= 0.20:
            return 'background-color: #e74c3c; font-weight: bold; color: white'
        elif val <= 0.22:
    ...
    # Apply styling
    styled = df.style\
        .applymap(color_auc, subset=['AUC'])\
        .applymap(color_accuracy, subset=['Accuracy'])\
        .applymap(color_brier, subset=['Brier'])\
        .format({'AUC': '{:.4f}', 'Accuracy': '{:.4f}', 'Precision': '{:.4f}',
                'Recall': '{:.4f}', 'F1': '{:.4f}', 'Brier': '{:.4f}', 'BalancedAcc': '{:.4f}', 'Specificity': '{:.4f}'})\
        .set_properties(**{'text-align': 'center', 'font-size': '12pt', 'border': '2px solid black'})

    return styled

# 2. CREATE TABLE FIGURE
def create_presentation_table(df_results):
    """Create a high-resolution table figure for presentations"""

    fig, ax = plt.subplots(figsize=(16, 10), dpi=150) # Increased figsize to accommodate more columns
    ax.axis('tight')
    ax.axis('off')

    # Prepare data for table (include new metrics)
    table_data = df_results[['AUC', 'Accuracy', 'Precision', 'Recall', 'F1', 'BalancedAcc', 'Specificity', 'Brier']].round(4)
    ...
    table = ax.table(cellText=table_data.values,
                    ...
                    colWidths=[0.1]*len(table_data.columns)) # Adjusted colWidths

    # Style the table
    table.auto_set_font_size(False)
    table.set_fontsize(10) # Adjusted font size
    table.scale(1.2, 2)
    ...
    legend_elements = [
        Patch(facecolor='#2ecc71', label='Excellent (AUC ≥ 0.79)'),
        Patch(facecolor='#3498db', label='Good (AUC ≥ 0.77)'),
        Patch(facecolor='#e74c3c', label='Best Brier (≤ 0.20)')
    ]
    plt.legend(handles=legend_elements, loc='upper left', bbox_to_anchor=(0, -0.1), ncol=3)
...
# 3. CREATE KEY METRICS CARD
def create_metrics_card(df_results):
    """Create a metrics card for presentation"""
    ...
    metrics = [
        ('Best AUC', df_results['AUC'].max(), df_results['AUC'].idxmax(), '#2ecc71'),
        ('Best Balanced Acc', df_results['BalancedAcc'].max(), df_results['BalancedAcc'].idxmax(), '#3498db'), # Added Balanced Accuracy
        ('Best Brier', df_results['Brier'].min(), df_results['Brier'].idxmin(), '#e74c3c'),
        ('Models Tested', len(df_results), '', '#9b59b6'),
        ('Above 0.77 AUC', (df_results['AUC'] > 0.77).sum(), '', '#1abc9c'),
        ('Avg Precision', df_results['Precision'].mean(), '', '#f39c12')
    ]

    for ax, (metric_name, metric_value, model_name, color) in zip(axes, metrics):
        ax.set_xlim(0, 10)
        ax.set_ylim(0, 10)
        ax.axis('off')

        # Add colored background
        rect = plt.Rectangle((0.5, 2), 9, 6, facecolor=color, alpha=0.3,
        ...
        if isinstance(metric_value, (int, np.integer)):
            ...
            value_text = f"{metric_value:.4f}"

        ax.text(5, 6, value_text, fontsize=36, fontweight='bold',
               ha='center', va='center', color=color)

        # Add metric name
        ax.text(5, 3.5, metric_name, fontsize=14, fontweight='bold',
               ha='center', va='center', color='#2c3e50')

        # Add model name
        if model_name:
            ax.text(5, 2.5, f"({model_name})", fontsize=10,
                   ha='center', va='center', color='#7f8c8d', style='italic')

    plt.suptitle('Key Performance Metrics', fontsize=20, fontweight='bold', y=0.98)
    plt.tight_layout()
    plt.savefig('key_metrics_card.png', dpi=300, bbox_inches='tight',
                facecolor='white', edgecolor='none')
    plt.show()

    return fig

# 4. CREATE RANKING VISUALIZATION
def create_ranking_chart(df_results):
    """Create a ranking chart for models"""
    ...
    bars = ax.barh(range(len(df_sorted)), df_sorted['AUC'].values)

    # Color bars based on performance
    colors = ['#2ecc71' if x >= 0.79 else '#3498db' if x >= 0.77 else '#95a5a6'
              for x in df_sorted['AUC'].values]
    ...
    for i, (idx, row) in enumerate(df_sorted.iterrows()):
        ax.text(row['AUC'] + 0.002, i, f"{row['AUC']:.4f}",
               va='center', fontweight='bold', fontsize=11)

    # Customize chart
    ax.set_yticks(range(len(df_sorted)))
    ax.set_yticklabels(df_sorted.index, fontsize=12, fontweight='bold')
    ax.set_xlabel('AUC Score', fontsize=14, fontweight='bold')
    ax.set_title('Model Performance Ranking by AUC', fontsize=16, fontweight='bold', pad=20)
    ax.set_xlim(df_sorted['AUC'].min() * 0.95, df_sorted['AUC'].max() * 1.05) # Adjusted xlim dynamically
    ax.grid(axis='x', alpha=0.3)

    # Add performance zones (adjust thresholds based on data range)
    ax.axvspan(0.79, ax.get_xlim()[1], alpha=0.1, color='green', label='Excellent (≥0.79)')
    ax.axvspan(0.77, 0.79, alpha=0.1, color='blue', label='Good (0.77-0.79)')
    ax.axvspan(ax.get_xlim()[0], 0.77, alpha=0.1, color='gray', label='Acceptable (<0.77)')


    ax.legend(loc='lower right', fontsize=10)

    plt.tight_layout()
    plt.savefig('model_ranking_chart.png', dpi=300, bbox_inches='tight',
                facecolor='white', edgecolor='none')
    plt.show()
...
# df_results is the results DataFrame
# The code above now populates the 'results' dictionary and creates df_results

# Generate all visualizations
styled_df = style_dataframe(df_results)
table_fig = create_presentation_table(df_results)
metrics_fig = create_metrics_card(df_results)
ranking_fig = create_ranking_chart(df_results)

# Save styled DataFrame as HTML
styled_df.to_html('styled_results_table.html')

# Create PowerPoint-ready summary
summary_for_ppt = pd.DataFrame({
    'Model': df_results.index,
    'AUC': df_results['AUC'].apply(lambda x: f"{x:.3f}"),
    'Rank': range(1, len(df_results) + 1)
})

print("\n✓ All visualizations saved:")
print("  - model_comparison_table.png (300 DPI)")
print("  - key_metrics_card.png (300 DPI)")
print("  - model_ranking_chart.png (300 DPI)")
print("  - styled_results_table.html")
