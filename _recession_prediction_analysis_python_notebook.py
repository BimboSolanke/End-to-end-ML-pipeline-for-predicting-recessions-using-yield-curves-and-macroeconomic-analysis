# -*- coding: utf-8 -*-
"""_RECESSION_PREDICTION_ANALYSIS python notebook.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/19y1QmDq_zVx4IhyET3l0SoSM-Z4XYNnc
"""
...
def load_comprehensive_data(start='1970-01-01', end='2024-12-31'):
    """
    Load all data from local CSV files already downloaded
    """
    print("\n" + "="*80)
    print("DATA COLLECTION FROM LOCAL FILES")
    print("="*80)
    ...
    # 1. LOAD TREASURY YIELDS FROM FRB_H15.csv
    try:
        print("\nLoading Treasury yields from FRB_H15.csv...", end=' ')
        h15_df = pd.read_csv('FRB_H15.csv')

        # First column should be dates - adjust if needed based on actual structure
        # Assuming first column is 'Series Description' with dates
        h15_df['DATE'] = pd.to_datetime(h15_df.iloc[:, 0])
        h15_df.set_index('DATE', inplace=True)

        # Map columns to standard FRED codes
        treasury_mapping = {
            'Market yield on U.S. Treasury securities at 3-month  constant maturity, quoted on investment basis': 'DGS3MO',
            'Market yield on U.S. Treasury securities at 2-year  constant maturity, quoted on investment basis': 'DGS2',
            'Market yield on U.S. Treasury securities at 5-year  constant maturity, quoted on investment basis': 'DGS5',
            'Market yield on U.S. Treasury securities at 10-year  constant maturity, quoted on investment basis': 'DGS10',
            'Market yield on U.S. Treasury securities at 30-year  constant maturity, quoted on investment basis': 'DGS30'
        }

        # Select and rename treasury columns
        for old_name, new_name in treasury_mapping.items():
            if old_name in h15_df.columns:
                treasury_data = h15_df[[old_name]].rename(columns={old_name: new_name})
                treasury_data = treasury_data[start_date:end_date]
                # Convert to numeric, replacing non-numeric values with NaN
                treasury_data[new_name] = pd.to_numeric(treasury_data[new_name], errors='coerce')
                all_data.append(treasury_data)
                print(f"\n  ✓ {new_name} loaded ({len(treasury_data)} observations)")

    except Exception as e:
        print(f"\n  ✗ Failed to load Treasury data: {str(e)}")
        failed_series.append('Treasury yields')

    # 2. LOAD OTHER INDICATORS FROM fred_data FOLDER
    other_series = {
        'AAA.csv': 'AAA',
        'BAA.csv': 'BAA',
        'UNRATE.csv': 'UNRATE',
    ...
    for filename, series_name in other_series.items():
        filepath = os.path.join('fred_data', filename)
        try:
            print(f"\nLoading {series_name} from {filepath}...", end=' ')

            # Read CSV - FRED files typically have DATE as first column
            ...
            if date_col:
                series_df['DATE'] = pd.to_datetime(series_df[date_col])
                series_df.set_index('DATE', inplace=True)

                # Get the value column (usually the second column or named after the series)
                value_col = None
                for col in series_df.columns:
                    if col != date_col and col not in ['DATE', 'Date', 'date']:
                        value_col = col
                        break

                if value_col:
                    # Rename to standard name
                    series_df = series_df[[value_col]].rename(columns={value_col: series_name})
                else:
                    # If only one column left after setting index, use it
                    if len(series_df.columns) == 1:
                        series_df.columns = [series_name]

                # Filter date range
                series_df = series_df[start_date:end_date]

                # Convert to numeric
                series_df[series_name] = pd.to_numeric(series_df[series_name], errors='coerce')

                all_data.append(series_df)
                print(f"✓ Success ({len(series_df)} observations)")
            else:
                print(f"✗ Failed: Could not find date column")
    ...
    if failed_series:
        print(f"\n⚠ Warning: Failed to load {len(failed_series)} series: {failed_series}")

    # 3. COMBINE ALL DATA
    if all_data:
        print("\n\nCombining all data series...")
    ...
    print(f"\nColumns loaded: {list(df_monthly.columns)}")
...
if __name__ == "__main__":
    try:
        ...
        print("\n✓ Consolidated data saved to 'recession_raw_data.csv'")
        ...
        print("Please check that all CSV files are in the correct locations:")
        print("  - FRB_H15.csv should be in the main directory")
        print("  - AAA.csv, BAA.csv, FEDFUNDS.csv, INDPRO.csv, UNRATE.csv, USREC.csv")
        print("    should be in the 'fred_data' folder")
...
if __name__ == "__main__":
    try:
        ...
        print("\n" + "="*80)
        print("STAGE 1 COMPLETE - Ready for Stage 2: EXPLORATORY DATA ANALYSIS (EDA)")
        print("="*80)
